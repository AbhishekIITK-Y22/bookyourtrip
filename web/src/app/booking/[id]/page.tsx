  ...
  return (
    <div className="rounded-xl border bg-white p-6 shadow-sm">
      ...
      <div className="mt-3 grid md:grid-cols-2 gap-3 text-sm text-gray-700">
        ...
        <div>Payment: {data.paymentState}</div>
      </div>
      <div className="flex gap-3 mt-6">
        {data.paymentState !== 'PAID' && (
          <button onClick={pay} disabled={paying} className="bg-green-600 hover:bg-green-700 disabled:opacity-60 text-white rounded px-4 py-2">{paying ? 'Processing...' : 'Pay Now'}</button>
        )}
        <button onClick={cancelBooking} disabled={cancelling} className="bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white rounded px-4 py-2">{cancelling ? 'Cancelling...' : 'Cancel'}</button>
      ...
      <div className="mt-6 rounded-xl border bg-white p-4 grid gap-2 max-w-lg">
        ...
        <div className="text-xs text-gray-500">Penalty may apply if &lt 24 hours to original departure.</div>
      </div>
    </div>
  );
}
